...
public:
    ...
    Eigen::Vector3d local2global(const Eigen::Vector3d &local_position, int iX, int iY, int iZ);

private:

    double find_yslice(double y_global);
    Eigen::Vector3d rotate_y(const Eigen::Vector3d &position, double &theta);
